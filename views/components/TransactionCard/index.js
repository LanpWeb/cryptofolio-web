...
const TransactionCard = ({
  order = 1, id = 1, name = "BTC", type = "Sell", date = "10.01.20", time = "22:08", price = "9,440.23", amount = "0.0021", worth = "10,999.92", editHandler = () => {}, deleteHandler = () => {}
}:Props) => {
    ...
    <div className="transaction-card aic jcsb">
      <div className="transaction-card__item aic">
        ...
        <div className="transaction-card__coin aic">
          <img src={`https://s2.coinmarketcap.com/static/img/coins/32x32/${id}.png`} alt="coin-logo" className="transaction-card__logo" />
          <span className="p2 fw-medium transaction-card__text transaction-card__text_acent">{name}</span>
        </div>
      </div>
      ...
      <div className="transaction-card__item transaction-card__item_sm transaction-card__btn-group aic jcfe">
        <button className="pure-btn transaction-card__btn" onClick={editHandler}><Edit /></button>
        <button className="pure-btn transaction-card__btn" onClick={deleteHandler}><Delete /></button>
      </div>
    </div>
  );
};
